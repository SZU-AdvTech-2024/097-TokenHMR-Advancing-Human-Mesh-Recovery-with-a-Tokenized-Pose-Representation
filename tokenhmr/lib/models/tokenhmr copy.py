...
from .mae import model_mae
from .heads import build_smpl_head
from .sapiens.depth import DepthModel
from .concat.feat_concat import get_concat
...
class TokenHMR(pl.LightningModule):
    ...
    def __init__(self, cfg: CfgNode, init_renderer: bool = True, is_train_state=False, is_demo=False):
        ...
        self.depthmodel = DepthModel(device='cuda:2')
        ...
        if not is_train_state:
            ...
            self.mae = model_mae.load_pretrained_mae()
            self.fusion = get_concat('concat_conv', 1024, 1280)  # 选择融合方式 'additive' 'concat_conv' 'multiplicative'
        ...
        # Model in training state
            ...
            self.mae = model_mae.load_pretrained_mae()
            self.mae.to("cuda:2")
            self.fusion = get_concat('concat_conv', 1024, 1280)
            self.fusion.to("cuda:2")
    ...
    def get_parameters(self):
        ...
        all_params += list(self.mae.parameters())
    ...
    def forward_step(self, batch: Dict, train: bool = False) -> Dict:
        ...
        # 计算深度图和 MAE 潜特
        depthbatch = self.depthmodel.depth_estimation_inference(x).to("cuda:0")

        mae_feats = model_mae.test_mae_input_output(self.mae, depthbatch)

        # 计算融合特征
        fusion_feats = self.fusion(conditioning_feats, mae_feats)

        # 计算 SMPL 参数
        pred_smpl_params, pred_cam, pred_smpl_params_list = self.smpl_head(fusion_feats)
    ...
    def compute_loss(self, batch: Dict, output: Dict, train: bool = True) -> torch.Tensor:
        ...
        if self.cfg.MODEL.LOOSE_SUP and train:
            ...
            loss_keypoints_2d = self.keypoint_2d_loss(pred_keypoints_2d, gt_keypoints_2d, weak_mask,
                                                      self.cfg.MODEL.LOOSE_WEIGHT)

            valid_3D_mask = torch.Tensor([name in ['H36M-TRAIN-WMASK', 'BEDLAM'] for name in dataset_names]).float().to(
                gt_keypoints_3d.device)
            ...
            for k, pred in pred_smpl_params.items():
                ...
                if k in ['betas']:
                    ...
                    valid_mask3D = angle_error > angle_valid_thresh[k][None].repeat(batch_size, 1).to(
                        angle_error.device)
                ...
                loss_smpl_params[k] = self.smpl_parameter_loss(pred, gt, has_gt, valid_mask3D, weak_mask,
                                                               self.cfg.MODEL.LOOSE_WEIGHT)
        else:
            # Compute 3D keypoint loss
