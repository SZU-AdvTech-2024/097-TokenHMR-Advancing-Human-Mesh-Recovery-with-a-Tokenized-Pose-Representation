...
def resize_bool_array(bool_array, target_size, interpolation=cv2.INTER_LINEAR):
    """
    将一个布尔数组等比缩放为指定大小。
...
def depth_estimation_inference(x, flip=False):
    """
    ...
    model = init_model(config=os.path.join(script_dir, 'sapiens_2b_render_people-1024x768.py'),
    ...
    for i in range(batch_size):
        ...
        cv2.imwrite('/home/vcc/Downloads/figure1.png', image)
        image_bgr = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)  # 转为 BGR 格式
        # 调用 get_person_mask_and_bbox 函数来获取 mask 和 bbox
        mask = get_person_mask(image)

        # 如果获取的 mask 为空或者 bbox 为空，您可以根据需要处理这些情况
        if mask is None:
            raise ValueError(f"Failed to retrieve mask or bbox for image: ", f"image_{i}.jpg")
        ...
        mask = resize_bool_array(mask, (256, 256))
        mask = mask > 0
